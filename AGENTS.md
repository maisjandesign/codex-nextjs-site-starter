...
At startup, load site-motion, the relevant GSAP skills and better-ui; record the header/hero/buttons/section motion map in BRIEF.md and implement the first working examples promptly. Generic advice to skip page-load animation, use CSS-only hover, or avoid all expressive motion must not cancel these explicitly required entrances and controls. Retain reduced-motion/static fallbacks and avoid distracting repeated motion in forms or dense utility controls.
