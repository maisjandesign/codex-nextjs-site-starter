---
name: better-ui
description: Refine shared UI surfaces, optical alignment, icons and component states while following the project design system.
---
...
Preserve the project's component library, tokens and density. Animation is specified only by docs/MOTION-DEFAULTS.md: upward masks, GSAP object hovers and ScrollSmoother.
...
1. **Verification**: list the exact checks run and their observed results. Walk every relevant state; use docs/MOTION-DEFAULTS.md for the retained GSAP behavior. If a check was not run, state what still needs verification.
