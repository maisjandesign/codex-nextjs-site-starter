...
This is optional reference guidance, not another motion contract. Follow MOTION-DEFAULTS.md first; choose pacing for its three defaults from the actual brief. Do not ask the user to select a technical stack or wait for another reminder to implement entrances, controls, and necessary imagery. Keep a short rationale in BRIEF.md and begin the desktop promptly. Supplied screenshots and mockups are the design to implement under AGENTS.md. Motion must preserve their composition, section inventory/order, content hierarchy, and interaction patterns; a scene concept does not authorize structural redesign.
...
Keep the previous galleries in MOTION-LIBRARIES.md as additional discovery sources. When research is useful, inspect relevant live examples; record exact URLs and the mechanism selected, not just a library name. Reference research must not postpone the first working desktop for a full research report.

## Engine and ownership decisions

GSAP + ScrollTrigger are the installed base. SplitText and Flip ship in the installed GSAP package and are used by the starter. Read official [SplitText](https://gsap.com/docs/v3/Plugins/SplitText/) and [Flip](https://gsap.com/docs/v3/Plugins/Flip/) docs for the current APIs. GSAP ScrollSmoother, masked element entrances and expressive button hovers are the three defaults under [MOTION-DEFAULTS.md](MOTION-DEFAULTS.md). Motion and Anime.js may supplement distinct effects; baseline replacement requires explicit user direction or approval.
...
One element/property has one motion owner. Use separate wrappers for an entrance and an independently interactive surface. Centralize timing/distance/easing, release contexts/listeners on unmount, and preserve text wrapping when fonts or widths change. The required browser observation in MOTION-DEFAULTS.md remains the completion gate; no engine or profile guarantees award-quality results.
