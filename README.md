...
GSAP and @gsap/react are installed, with reusable ScrollTrigger rise/stagger/mask patterns and replayable examples in /design-system. Use **docs/MOTION-LIBRARIES.md** to select distinctive effects from the supplied sources for each project. Simple CSS feedback remains useful, but it is no longer the complete motion layer. **docs/IMAGE-ASSETS.md** requires generating needed high-resolution imagery from visual references, checking actual output size, and integrating optimized local assets instead of screenshot crops. No unrelated sample imagery was generated for this template.
